# -*- coding: utf-8 -*-
"""111–112 上站：中英 4 个词条页接入视频版（YouTube 无 Cookie 嵌入）与图文幻灯图集。
...
ENTRIES = [  # 序号, slug, 中文名, 英文名, 中文ID, 英文ID
    (111, "global-government", "全球政府", "Global Government", "cU4wqj3Ovbc", "VPECiPQsIck"),
    (112, "heavenly-bank", "天国银行", "Heavenly Bank", "DfqQ0x6UXrk", "XRO9TDZjUy0"),
]
...
for ep, slug, zh, en, cid, eid in ENTRIES:
    for lang, name, vid in (("zh", zh, cid), ("en", en, eid)):
        ...
        nav = next((i for i, l in enumerate(lines) if re.fullmatch(r"## (版本导航|Versions|Version Navigation)\s*", l)), None)
