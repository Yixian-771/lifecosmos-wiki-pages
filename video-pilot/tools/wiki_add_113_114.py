# -*- coding: utf-8 -*-
"""113–114 上站：中英 4 个词条页接入视频版（YouTube 无 Cookie 嵌入）与图文幻灯图集。
图集从「百科图片版」当前无水印图重出（宽 1280、JPEG q86，与 resync_slides.py 同规格），保证是修补后的版本。
视频段插入位置：有「版本导航 / Versions / Version Navigation」的插在它前面；没有的插在「相关词条 / Related Entries（或 entries）」前面。
...
ENTRIES = [  # 序号, slug, 中文名, 英文名, 中文ID, 英文ID
    (113, "lifecosmos-money-view", "生命禅院金钱观", "Lifechanyuan's View on Money", "O95mtFYbydw", "dWPWhWEvsL4"),
    (114, "civilization-overview", "文明（总论）", "Civilization: An Overview", "iydkQXq1jEI", "qSwdHI2t8nM"),
]
...
for ep, slug, zh, en, cid, eid in ENTRIES:
    for lang, name, vid in (("zh", zh, cid), ("en", en, eid)):
        ...
        nav = next((i for i, l in enumerate(lines) if re.fullmatch(r"## (版本导航|Versions|Version Navigation|Read by Edition)\s*", l)), None)
        rel = next((i for i, l in enumerate(lines) if re.fullmatch(r"## (相关词条|Related [Ee]ntries)\s*", l)), None)
        if nav is not None:
